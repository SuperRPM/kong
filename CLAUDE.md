...
## 셀프호스팅 배포 (사내망, `.gitlab-ci.yml`)
Vercel과 별도로, 사내망 전용 self-hosted Supabase + Docker 배포가 `192.168.20.87`에서 운영됨.

| 포트 | 역할 | 접근 주체 |
|------|------|-----------|
| 3000 | Next.js 앱(`kong-app`) 원시 포트 | 보통 직접 안 씀 |
| 80 | Nginx 리버스 프록시 → 내부 3000 | 정식 접속 진입점 (`http://192.168.20.87`) |
| 8000 | Supabase API 게이트웨이(supabase-kong) | **브라우저가 로그인 시 직접 호출** |

- `NEXT_PUBLIC_SUPABASE_URL`은 브라우저가 로그인/데이터 요청 시 **직접** 호출하는 주소이며 **빌드 타임에 클라이언트 번들에 박제(baked)**됨 — 값을 바꾸면 재빌드 필요. 현재 `http://192.168.20.87:8000`(사내 IP)로 설정, hosts 등록 없이 사내망 어디서든 접속 가능하도록 유지할 것.
- 호스트명(`kong.mww` 등)으로 바꾸는 경우, **브라우저뿐 아니라 컨테이너 내부**에서도 그 이름이 풀려야 함(서버사이드 `getUser` 검증도 같은 값을 씀) — 컨테이너는 `--add-host`로 별도 매핑 필요.
- 사내 LAN 전용 hosts 매핑: `192.168.20.87 kong.mww` (원격/Tailscale용 매핑은 별도 인원 한정, 여기 기록하지 않음)
- 로그인/접속 장애 시 빠른 점검: ① `docker ps`로 컨테이너 healthy 확인 ② `curl http://192.168.20.87:8000/auth/v1/settings -H "apikey:<anon>"`=200 확인 ③ 브라우저 Network 탭에서 `token?grant_type=password` 상태 확인 — `ERR_NAME_NOT_RESOLVED`면 주소 미해석, `400`이면 진짜 자격증명 오류, pending이면 도달 불가.
